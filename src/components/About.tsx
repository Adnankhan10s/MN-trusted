...
import AboutHead from './otherComponents/AboutHead';
import AboutCard from './otherComponents/AboutCard';
...
const About = () => {
  return (
    <>
      <div className='py-2 border-b-2 border-[#00004f] w-full md:h-[1200px] sm:h-[1700px] h-[1700px] bg-slate-200 border-t-2 border-slate-400 shadow-inner'>
        <AboutHead />
        ...
        <p className='pt-2'>Our team offers expertise in a wide range of services, including:
         ...
         <ul className='pt-6 text-start px-14 leading-8 space-y-4 '>
           {services.map((index, value) => (
             <li key={value} className='md:flex  inline-flex items-center gap-4 text-[10px]  md:text-lg'><span><FaRegCircleCheck className='text-[#f99b27] md:size-8' size={20} /></span>   {index.title}</li>
          ))}
